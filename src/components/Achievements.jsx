...
export default function Achievements() {
  ...
  useAnimationFrame((time, delta) => {
    if (!isHovered.current && trackRef.current) {
      // Move 1px per 16ms (approx 60fps)
      const moveBy = (delta / 16) * 0.8;
      x.current -= moveBy;
      
      const halfWidth = trackRef.current.scrollWidth / 2;
      // If we've scrolled past half the width (one full set), seamlessly reset
  ...
  const scroll = (direction) => {
    if (trackRef.current) {
      isHovered.current = true; // Pause auto scroll
      ...
      // Wrap around seamlessly if jumping out of bounds
      if (x.current > 0) x.current -= halfWidth;
      if (x.current <= -halfWidth) x.current += halfWidth;
      
      trackRef.current.style.transform = `translateX(${x.current}px)`;
      
      // Resume auto scroll after smooth transition finishes
      setTimeout(() => {
        if (trackRef.current) {
